...
Gem det, du har i dit R-projekt. Lav et nyt projekt til Statistical Learning. Kald det evt. ITSL2. Gem alle kode-filer samme sted.

Kør følgende kode, som laver et sub-directory i dit working directory ved navn "data". Gem alle data til bogen (ligger på Moodle under data til bogen) i dette directory.
...
Følgende er ikke relevant, da vi bruger source pane og ikke console pane:
...
```{r chunk3}
length(x) # men du kan jo også se det hele under Global Environment
length(y) 
x + y
```
...
```{r chunk4}
ls() # men du kan jo også se det hele under Global Environment
rm(x, y)
ls()
```
...
Code Chunk 17 og 18 ikke så relevant.
